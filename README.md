...
Raw Harbor jobs remain under ignored local paths. Every execution writes an allowlisted `Run_Report.json` beside its manifest and refreshes the ignored local dashboard once after the run, including after a partial failure. Only reviewed, finalized files produced through `harness-test result sanitize` may enter `results/`; publication also requires a content-valid current-series manifest and no reviewed mapping. Old hidden-contract and plugin-seed cohorts remain local and quarantined, with no regrade or mapping into current schema `0.3.0`. The Observable Framework dashboard keeps local run status separate from the six public result views: Latest, Trends, Comparisons, Task matrix, Run detail, and Quality versus efficiency.
