...
Read `/app/Records.json`. The public contract is available through `harness-stub describe` if needed. Use `harness-stub ACTION 'JSON_OBJECT'` to request one `quick` transformation into `/app/Summary.json` and directly verify its structure and traceability.
